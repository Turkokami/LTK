...
|---|---|---|---|---|
| 1 | State wildlife / nuisance-animal permit rules (TX, WA, FL, CA, SC) + federal bird rules (MBTA, falconry abatement) | Wildlife, bird and falconry field guides — replaces "state-by-state guide in the works" | — | **In progress (research agent)** |
| 2 | Lab technology explainers — 21 more to reach 25 | `lib/content/lab.ts` → `TECHNOLOGY` (shape: `TechnologyTopic`) | — | **4 in progress (research agent)**. Remaining topics: sensor networks for structural pests, cold/cryogenic treatment, electronic termite monitoring, exclusion materials (copper mesh, hardware cloth, steel wool vs. alternatives), rodenticide formulations and secondary-poisoning rules, bed bug heat vs. chemical, insect light traps, moisture meters and thermal imaging for WDO, mosquito trap surveillance, IGRs, fumigant monitoring equipment, field service software categories. |
